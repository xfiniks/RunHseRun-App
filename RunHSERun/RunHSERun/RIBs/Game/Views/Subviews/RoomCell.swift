...
final class RoomCell: UITableViewCell {
    ...
    func configure(with viewModel: ViewModel) {
        ...
        titleLabel.text = viewModel.code

        id = viewModel.id
        relatedRoom = viewModel
    }
    ...
    private func selectRoom() {
        guard let relatedRoom = relatedRoom, let completion = completion else {
            return
        }

        completion(relatedRoom.id)
    }
